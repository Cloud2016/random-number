...
knitr::opts_chunk$set(echo = TRUE,cache = TRUE,engine.path = list(
  octave = 'C:/Octave/Octave-4.2.1/bin/octave-cli.exe',
...
```{r,fig.width=4,fig.height=2.8,fig.align='center',cache=TRUE,message=FALSE}
set.seed(1234)
corr <- rep(0,1000) 
...
我们现在拿Matlab早期随机数发生器、R内置的Mersenne-Twister发生器和C语言实现的Mersenne-Twister发生器比较，程序实现仿Matlab 版的游程检验^[<https://www.mathworks.com/content/dam/mathworks/mathworks-dot-com/moler/random.pdf>]。

```{r,fig.height=3,fig.width=9,fig.align='center'}
set.seed(1234) # R默认采用Mersenne Twister发生器
n <- 2^24; 
x <- runif(n,0,1); # R内生成均匀分布随机数
delta <- 0.01 
len <- diff(c(0,which(x<delta),n+1))-1 
p1 <- ggplot(data.frame(x=len[len < 101]),aes(x,fill=..count..)) + 
	scale_fill_viridis(direction = -1)+ 
	geom_histogram(binwidth = 1,show.legend = FALSE) + 
	xlab("R")+ylab("") 
dat <- readMat("random_number.mat") # 读取Octave生成的均匀分布随机数  
x <- dat$x 
len <- diff(c(0,which(x<delta),n+1))-1 
p2 <- ggplot(data.frame(x=len[len < 101]),aes(x,fill=..count..)) + 
	scale_fill_viridis(direction = -1)+ 
	geom_histogram(binwidth = 1,show.legend = FALSE) + 
	xlab("Matlab v5/Octave")+ylab("") 
y <- read.table(file = "random_number.txt") # 读取C语言生成的均匀分布随机数 
y <- c(as.matrix(t(y)))
len <- diff(c(0,which(y<delta),n+1))-1 
p3 <- ggplot(data.frame(x=len[len < 101]),aes(x,fill=..count..)) + 
	scale_fill_viridis(direction = -1)+ 
	geom_histogram(binwidth = 1,show.legend = FALSE) + 
	xlab("C")+ylab("") 
...
在游程长度为27的位置，有一条深沟，这是George Marsaglia 提出的借位相减（subtract-with-borrow）算法的特性，显然不符合随机性的要求^[<https://www.mathworks.com/content/dam/mathworks/mathworks-dot-com/moler/random.pdf>]。
...
```{r,fig.width=4,fig.height=2.8,fig.align='center'}
...
一般地，$n$个独立随机变量的和
$$\varphi_n(t)=\big(\frac{e^{it}-1}{it}\big)^n=\big(\frac{\sin^{n}(t/2)\mathrm{e}^{\frac{it}{2}}}{t/2}\big)^n$$
那么，同理 
